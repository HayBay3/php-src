...
static int curl_sockopt(void *ctx, curl_socket_t curlfd, curlsocktype purpose)
{
	...
	switch (t->method) {
		case PHP_CURL_USER: {
			...
			ZVAL_RES(&argv[0], ch->res);
			Z_ADDREF(argv[0]);
			php_sock = socket_import_file_descriptor(curlfd);
...
int _php_curl_addr_to_zval(struct curl_sockaddr *address, zval *native_addr)
{
	...
	switch (addr->sa_family) {
		case AF_INET: {
			...
			char addr4_str[INET_ADDRSTRLEN+1];

			if (!inet_ntop(addr->sa_family, &sin->sin_addr, addr4_str, INET_ADDRSTRLEN)) {
				return 1;
			}
			add_next_index_string(native_addr, addr4_str);
			add_next_index_long(native_addr, ntohs(sin->sin_port));
			break;
		}
		case AF_INET6: {
			...
			if (!inet_ntop(addr->sa_family, &sin6->sin6_addr, addr6_str, INET6_ADDRSTRLEN)) {
				return 1;
			}
			add_next_index_string(native_addr, addr6_str);
			add_next_index_long(native_addr, ntohs(sin6->sin6_port));
			add_next_index_long(native_addr, ntohl(sin6->sin6_flowinfo));
			add_next_index_long(native_addr, ntohl(sin6->sin6_scope_id));
			break;
		}
		case AF_UNIX: {
			struct sockaddr_un *s_un = (struct sockaddr_un *)addr;
			add_next_index_string(native_addr, s_un->sun_path);
			break;
		}
		default: {
...
static curl_socket_t curl_opensocket(void *ctx, curlsocktype purpose, struct curl_sockaddr *address)
{
	...
	switch (t->method) {
		case PHP_CURL_USER: {
			...
			if (_php_curl_addr_to_zval(address, &php_addr)) {
				...
				if (error == FAILURE) {
					...
					if (Z_TYPE_P(&retval) == IS_RESOURCE) {
						php_sock = (php_socket *)zend_fetch_resource2_ex(&retval, NULL,
									php_sockets_le_socket(), php_sockets_le_socket());
						if (php_sock) {
